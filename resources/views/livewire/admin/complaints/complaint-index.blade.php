<div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900">Complaints</h1>
            <p class="mt-1 text-sm text-slate-500">Complaints submitted from the public website.</p>
        </div>
                    ...
                    <div class="flex min-w-0 flex-1 items-center gap-4">
                        ...
                        <div class="min-w-0">
                            <div class="flex flex-wrap items-center gap-2">
                                <p class="truncate text-sm font-semibold text-slate-900">{{ $complaint->full_name }}</p>
                                <span class="text-sm text-slate-400">·</span>
                                <p class="text-sm text-slate-500">{{ $complaint->email }}</p>
                                <span class="text-xs text-slate-400">· {{ $complaint->messages_count }} message(s)</span>
                            </div>
                            <p class="mt-1 line-clamp-2 text-sm text-slate-600">{{ $complaint->description }}</p>
                            <p class="mt-1 text-xs text-slate-400">{{ $complaint->created_at->format('d M Y H:i') }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        <x-badge :color="match($complaint->status->value) { 'new' => 'blue', 'open' => 'amber', 'flagged' => 'red', 'resolved' => 'green', default => 'gray' }">
                            {{ ucfirst($complaint->status->value) }}
                        </x-badge>
                        <x-button size="sm" variant="secondary" wire:click="openReply({{ $complaint->id }})">Reply</x-button>
                        <x-button size="sm" variant="secondary" wire:click="setStatus({{ $complaint->id }}, 'resolved')">Resolve</x-button>
                    </div>
                </li>
            @empty
                <li class="px-5 py-16">
                    <x-empty-state title="No complaints" description="Complaints submitted from the website will appear here."/>
                </li>
            @endforelse
        </ul>
    ...
    <x-modal :show="'showReplyModal'" max-width="xl">
        <x-slot:title>Reply to Complaint</x-slot:title>
        <div>
