<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-white">Edit {{ $agreement->agreement_number }}</h1>
    ...
    @if ($isSigned)
        <div class="flex items-start gap-3 rounded-xl border border-amber-500/20 bg-amber-500/10 p-4">
            ...
            <div>
                <p class="text-sm font-semibold text-amber-300">Signed Agreement</p>
                <p class="mt-1 text-xs text-amber-300/70">A new version will be created and a fresh secure link generated. The client will receive the updated agreement by email.</p>
            </div>
        </div>
    @endif
    ...
    <form wire:submit="save" class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="space-y-6 lg:col-span-2">
            <div class="rounded-2xl border border-purple-500/15 bg-gradient-to-br from-white/[0.04] to-white/[0.01] p-6 shadow-lg shadow-purple-950/30 backdrop-blur-sm">
                <div>
                    <h2 class="text-lg font-bold text-white">Edit Agreement</h2>
                    <p class="mt-1 text-sm text-slate-400">Update the details below. Changes will be reflected in the current version.</p>
                </div>
                ...
                <div class="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <div
                            x-data="{
                                open: false,
                                search: '',
                                selectedCode: '{{ $client_mobile_dial ?? '+44' }}',
                                selectedFlag: '🇬🇧',
                    ...
                    {{-- Payment dropdown (read-only) --}}
                    <div class="sm:col-span-2">
                        <label class="mb-1.5 block text-sm font-medium text-slate-300">
                            Payment Type
                        </label>
                        <div class="flex items-center gap-3 rounded-lg bg-slate-900/40 px-3.5 py-2.5 ring-1 ring-inset ring-purple-500/10">
                            @php
                                $paymentMeta = match($payment_type) {
                                    'none' => ['icon' => 'M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636', 'bg' => 'bg-slate-500/15 text-slate-300 ring-slate-500/30', 'label' => 'No Payment Required'],
                                    'full' => ['icon' => 'M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z', 'bg' => 'bg-purple-500/15 text-purple-300 ring-purple-500/30', 'label' => 'Full Payment'],
                                    'milestone' => ['icon' => 'M3 3v1.5M3 21v-6m0 0l2.77-.693a9 9 0 016.208.682l.108.054a9 9 0 006.086.71l3.114-.732a48.524 48.524 0 01-.005-10.499l-3.11.732a9 9 0 01-6.085-.711l-.108-.054a9 9 0 00-6.208-.682L3 4.5M3 15V4.5', 'bg' => 'bg-orange-500/15 text-orange-300 ring-orange-500/30', 'label' => 'Milestone Payment'],
                                    'subscription' => ['icon' => 'M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99', 'bg' => 'bg-emerald-500/15 text-emerald-300 ring-emerald-500/30', 'label' => 'Subscription Payment'],
                                };
                            @endphp
                            <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-md ring-1 ring-inset {{ $paymentMeta['bg'] }}">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="{{ $paymentMeta['icon'] }}"/>
                                </svg>
                            </span>
                            <span class="text-sm font-semibold text-slate-200">{{ $paymentMeta['label'] }}</span>
                        </div>
                        <p class="mt-1.5 text-xs text-slate-500">Payment type cannot be changed after creation</p>
                    </div>
        ...
        <div class="space-y-6">
            <div class="rounded-2xl border border-purple-500/15 bg-gradient-to-br from-white/[0.04] to-white/[0.01] p-5 shadow-lg shadow-purple-950/30 backdrop-blur-sm">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-sm font-bold text-white">After saving changes</h3>
                </div>
                <ul class="mt-3 space-y-2 text-xs text-slate-400">
                    @if ($isSigned)
                        <li class="flex items-start gap-2">
                            <svg class="mt-0.5 h-3.5 w-3.5 shrink-0 text-amber-400" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z"/>
                            </svg>
                            A new version will be created (the signed version stays immutable)
                        </li>
                        <li class="flex items-start gap-2">
                            <svg class="mt-0.5 h-3.5 w-3.5 shrink-0 text-amber-400" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z"/>
                            </svg>
                            A fresh secure link will be generated
                        </li>
                        <li class="flex items-start gap-2">
                            <svg class="mt-0.5 h-3.5 w-3.5 shrink-0 text-amber-400" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z"/>
                            </svg>
                            The client will receive the updated agreement by email
                        </li>
                    @else
                        <li class="flex items-start gap-2">
                            ...
                            Changes will be saved to the current pending version
                        </li>
                        <li class="flex items-start gap-2">
                            <svg class="mt-0.5 h-3.5 w-3.5 shrink-0 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/>
                            </svg>
                            The existing secure link remains valid
                        </li>
                        <li class="flex items-start gap-2">
                            <svg class="mt-0.5 h-3.5 w-3.5 shrink-0 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/>
                            </svg>
                            No new version is created — this edit is in place
                        </li>
                    @endif
                </ul>
            </div>
